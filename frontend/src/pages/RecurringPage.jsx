    ...
    <div className="flex flex-col gap-5">
      ...
      {isLoading ? <Spinner /> : items.length ? (
        <div className="bg-white rounded-xl border border-gray-200">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between py-3 px-4 border-b border-gray-100 last:border-0">
              <div>
                <p className="font-medium text-gray-900">{item.description} — {formatAmount(item.amount)}</p>
              ...
              <Button variant="ghost" size="sm" onClick={() => { if (confirm('삭제?')) remove(item.id) }} className="text-red-500">삭제</Button>
            </div>
          ))}
        </div>
      ) : <EmptyState message="등록된 반복 지출이 없습니다." />}
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title="반복 지출 추가">
        <form onSubmit={handleSubmit(create)} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none" {...register('payment_method', { required: true })}>
          ...
          <div className="flex flex-col gap-1">
            ...
            <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none" {...register('cycle', { required: true })}>
